...
''')
    script.append('from forcefield_water import water_selection, ff_selection, generate_forcefield')
    script.append('from protein_ligand_prep import prepare_protein, protein_choice, prepare_ligand, rdkit_to_openmm, merge_protein_and_ligand')
    script.append('from post_md_conversions import mdtraj_conversion, MDanalysis_conversion')
    ...
    if fileType == 'pdb':
        ...
        if forcefield == 'AMOEBA2018':
            water = ('amoeba2018_gk.xml' if water == 'implicit' else None)
        elif forcefield == 'CHARMMPOLAR2019':
            water = None
        #elif water == 'implicit':
    ...
    script.append("water = '%s'" % session['waterModel'])
    ...
    if session['cleanup'] == 'yes':
        ...
        #script.append("Prepare_system = False")
        #script.append("remove_chains = False")
        #script.append("chain_ids = []")
    elif session['cleanup'] == 'no' and session['ligand_select'] == 'yes':
            script.append('''\n############# Membrane Settings ###################\n''')
            script.append("membrane_lipid_type = ''")
            script.append("membrane_padding = ")
            script.append("membrane_ionicstrength = ")
            script.append("membrane_positive_ion = ''")
            script.append("membrane_negative_ion = ''")
    ...
    if session['cleanup'] == 'no' and session['ligand_select'] == 'no':
        if session['manual_sys'] == 'no':
           script.append('''
...
forcefield = generate_forcefield(protein_ff=forcefield_selected, solvent_ff=water_selected, rdkit_mol=None)
...
protein_pdb = protein_choice(protein_pre=protein_prepared,protein=protein, chains=remove_chains, chain_ids=chain_ids)
...
if add_membrane == True:
    modeller.addMembrane(forcefield, lipidType= membrane_lipid_type, minimumPadding= membrane_padding * unit.nanometer, positiveIon=membrane_positive_ion, negativeIon=membrane_negative_ion, ionicStrength=membrane_ionicstrenght * unit.molar)
    print(f"Protein with Membrane {membrane_lipid_type} prepared")
    with open(f'membrane_{protein}', 'w') as outfile:
    	PDBFile.writeFile(modeller.topology, modeller.positions, outfile)
    ...
    with open(f'waterbox_{protein}', 'w') as outfile:
    	PDBFile.writeFile(modeller.topology, modeller.positions, outfile)
else:
    print("Error occured during membrane preparation")
...
if ligand = True:
    ...
    if protein_prepared == "No":
        with open(f'pdbfixed_{protein}', 'w') as outfile:
    ...
    forcefield = generate_forcefield(protein_ff=forcefield_selected, solvent_ff=water_selected, rdkit_mol=ligand_prepared)
    
    print(forcefield)
    
    modeller = app.Modeller(complex_topology, complex_positions)
    with open(f'prepared_no_box{protein}', 'w') as outfile:
    PDBFile.writeFile(modeller.topology, modeller.positions, outfile)
    
    if add_membrane == True:
        modeller.addMembrane(forcefield, lipidType= membrane_lipid_type, minimumPadding= Box_padding * unit.nanometer, ionicStrength= Ionic_Strenght * unit.molar)
        print(f"Protein with Membrane {membrane_lipid_type} prepared")
        with open(f'membrane_{protein}', 'w') as outfile:
            PDBFile.writeFile(modeller.topology, modeller.positions, outfile)
    elif add_membrane == False:
        if Water_Box == "Buffer":
            modeller.addSolvent(forcefield, padding=Box_padding * unit.nanometers, ionicStrength=Ionic_Strenght * unit.molar)
            print(f"Protein with buffer solvent prepared")
        elif Water_Box == "Absolute":
            modeller.addSolvent(forcefield, boxSize=Vec3(Water_Box_x, Water_Box_y, Water_Box_z) * unit.nanometers, ionicStrength=Ionic_Strenght * unit.molar)
        with open(f'solvent_{protein_name}', 'w') as outfile:
            PDBFile.writeFile(modeller.topology, modeller.positions, outfile)
        else:
            print("Error occured during membrane preparation")
