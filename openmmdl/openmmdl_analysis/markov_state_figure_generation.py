...
class MarkovChainAnalysis:
    ...
    def generate_transition_graph(
        ...
        part2_data: List[str] = combined_dict["all"][
            part1_length : part1_length + part2_length
        ]
        ...
        top_10_nodes: List[str] = sorted(
            node_occurrences, key=node_occurrences.get, reverse=True
        )[:10]

        for min_transition_percent in self.min_transitions:
            min_prob: float = (
                min_transition_percent / 100
            ...
            for transition, count in transitions.items():
                ...
                transition_probabilities_forward[forward_transition] = (
                    forward_count / node_occurrences[start_state]
                ) * 100
                transition_occurrences_forward[forward_transition] = (
                    forward_count / len(combined_dict["all"])
                ) * 100

                transition_probabilities_backward[backward_transition] = (
                    backward_count / node_occurrences[end_state]
                ) * 100
                transition_occurrences_backward[backward_transition] = (
                    backward_count / len(combined_dict["all"])
                ) * 100
            ...
            for u, v, data in G.edges(data=True):
                ...
                if u == v:  # Check if it is a self-loop
                    ...
                    edge_colors.append(
                        "black"
                    )  # Highlight significant transitions in black

                    # Check if both nodes are present before adding labels
                    if G.has_node(u) and G.has_node(v):
                        width: float = 4.0
                        forward_label: str = (
                            f"{transition_occurrences_forward.get((v, u), 0):.2f}% of Frames →, {transition_probabilities_forward.get((v, u), 0):.2f}% probability"
                        )
                        backward_label: str = (
                            f"{transition_occurrences_backward.get((u, v), 0):.2f}% of Frames ←, {transition_probabilities_backward.get((u, v), 0):.2f}% probability"
                        )
                    ...
                    edge_colors.append(
                        "grey"
                    )  # Use grey for non-significant transitions
                    width: float = 0.5
            ...
            node_size: List[int] = [
                size_node * node_occurrences[node] for node in G.nodes()
            ]
            ...
            for node in G.nodes():
                if G.degree(node) > 0:  # Check if the node has at least one edge
                    ...
                    if relevant_edges:
                        if node in top_10_nodes:
                            node_occurrence_percentage: float = (
                                node_occurrences[node] / len(combined_dict["all"])
                            ) * 100
                            self_loop_probability: float = (
                                self_loop_probabilities.get(node, 0) * 100
                            )
                            self_loop_occurence: float = self_loop_occurences.get(
                                node, 0
                            )
                            node_label: str = (
            ...
            # Save the plot
            plot_filename: str = (
                f"markov_chain_plot_{min_transition_percent}.{fig_type}"
            )
