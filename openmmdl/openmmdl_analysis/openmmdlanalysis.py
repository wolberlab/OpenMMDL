...
def parse_xyz(val):
    """
    Robustly parse (x,y,z) from strings like:
      "(1.0, 2.0, 3.0)"
        ...
        with mda.Writer(trajectory, n_atoms=u.atoms.n_atoms) as W:
            ...
            for ts in u.trajectory:
                if not first_frame_saved:
                    with mda.Writer(topology, n_atoms=u.atoms.n_atoms) as pdb_writer:
        ...
        with pushd(outdir):
            ...
            if generate_representative_frame:
                ...
                for mode in tqdm(modes_to_process):
                    ...
                    representative_frame = rmsd_analyzer.calculate_representative_frame(all_frames, DM)
