...
def perform_clustering_and_writing(stable_waters, cluster_eps=1.0, min_samples=1500):
    def write_pdb_clusters_and_representatives(clustered_waters):
        ...
        with pd.option_context('display.max_rows', None):  # Temporarily set display options
            for label, cluster in clustered_waters.groupby("Cluster_Label"):
                ...
                output_filename = f"cluster_{label}.pdb"
...
def process_trajectory_and_cluster(water_type, cluster_eps=1.0, min_samples=1500):
    # Load the PDB and DCD files
    u = mda.Universe("2.pdb", "2.dcd")
    ...
    # Save the stable_waters DataFrame to a CSV file
    stable_waters.to_csv(f"stable_waters_{water_type}.csv", index=False)

    # Call the clustering and writing function with the stable_waters DataFrame
    perform_clustering_and_writing(stable_waters, cluster_eps, min_samples)

# Call the function with the desired water type
# process_trajectory_and_cluster("T4P")

# process_trajectory_and_cluster("SPC")
# process_trajectory_and_cluster("T3P")
...
# Example usage
#protein_pdb_file = "2.pdb"
#representative_waters_file = "representative_waters.pdb"
#analyze_protein_and_water_interaction(protein_pdb_file, representative_waters_file)
