...
def generate_md_pharmacophore_cloudcenters(
    df, core_compound, output_filename, sysname, id_num=0
):
    ...
    for interaction in [
        ...
        if interaction in ["Acceptor_hbond", "Donor_hbond"]:
            pharm = generate_pharmacophore_vectors(
                df, df.filter(regex=interaction).columns
            )
            ...
            pharm = generate_pharmacophore_centers(
                df, df.filter(regex=interaction).columns
            )
            for feature_name, position in pharm.items():
                feature_id_counter += 1
            ...
            normal = ET.SubElement(
                plane, "normal", x3=str(x), y3=str(y), z3=str(z), tolerance="0.43633232"
            )
