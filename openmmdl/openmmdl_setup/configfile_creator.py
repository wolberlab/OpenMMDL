...
class ConfigCreator:
    ...
    def __init__(self, session: Dict[str, str], uploadedFiles: Dict[str, List[tuple[str, str]]]):
        """
    ...
    def add_amber_file_configuration(self, script: List[str]) -> None:
        ...
        if self.session["fileType"] == "amber":
            ...
            if self.session["has_files"] == "yes":
                ...
                nmLigName: Optional[str] = self.session["nmLigName"] if self.session["nmLig"] else None
                spLigName: Optional[str] = self.session["spLigName"] if self.session["spLig"] else None
        ...
        if self.session["fileType"] == "pdb":
            if self.session["solvent"]:
                if self.session["add_membrane"]:
                    ...
                    script.append(
                        "membrane_lipid_type = '%s'" % self.session["membrane_lipid_type"]
                    )
                else:
                    script.append(
                        "\n############# Solvent Settings ###################\n"
                    ...
                    script.append("ionic_strength = %s" % self.session["ionic_strength"])
