...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', dest='topology', help='Topology File after MD Simulation', required=True)
    parser.add_argument('-d', dest='trajectory', help='Trajectory File in DCD Format', required=True)
    parser.add_argument('-s', dest='ligand_sdf', help='Ligand in SDF Format', required=True)       
    parser.add_argument('-li', dest='ligand', help='Ligand Name (3 Letter Code in PDB)', required=True)
    ...
    for key, values in columns_with_value_1.items():
        ...
        highlighted_hbond_donor, highlighted_hbond_acceptor, highlighted_hbond_both, highlighted_hydrophobic, highlighted_waterbridge, highlighted_pistacking, highlighted_halogen, highlighted_saltbridge, highlighted_pication, highlighted_metal = highlight_numbers(split_data, starting_idx=lig_index)
        ...
        highlighted_saltbridge = [int(x) for x in highlighted_saltbridge]
        highlighted_pication = [int(x) for x in highlighted_pication]
        highlighted_metal = [int(x) for x in highlighted_metal]
    
    
        # Generate a dictionary for the first list with values (1, 0, 0)
        hbond_acceptor_dict = {key: (1.0,0.6,0.6) for key in highlighted_hbond_acceptor}

        # Generate a dictionary for the first list with values (1, 0, 0)
        hbond_both_dict = {key: (0.3,0.3,0.6) for key in highlighted_hbond_both}

        # Generate a dictionary for the first list with values (1, 0, 0)
        hbond_donor_dict = {key: (0.3,0.5,1) for key in highlighted_hbond_donor}

        # Generate a dictionary for the second list with values (0.8, 1, 0)
        hydrophobic_dict = {key: (0.8, 1, 0) for key in highlighted_hydrophobic}

        # Generate a dictionary for the second list with values (0.8, 1, 0)
        waterbridge_dict = {key: (0.8, 1, 0) for key in highlighted_waterbridge}

        # Generate a dictionary for the second list with values (0.8, 1, 0)
        pistacking_dict = {key: (0.8, 1, 0) for key in highlighted_pistacking}

        # Generate a dictionary for the second list with values (0.8, 1, 0)
        halogen_dict = {key: (0.8, 1, 0) for key in highlighted_halogen}

        # Generate a dictionary for the second list with values (0.8, 1, 0)
        saltbridge_dict = {key: (0.8, 1, 0) for key in highlighted_saltbridge}

        # Generate a dictionary for the second list with values (0.8, 1, 0)
        pication_dict = {key: (0.8, 1, 0) for key in highlighted_pication}

        # Generate a dictionary for the second list with values (0.8, 1, 0)
        metal_dict = {key: (0.8, 1, 0) for key in highlighted_metal}
        ...
        hbond_donor_dict.update((k, v) for k, v in saltbridge_dict.items() if k not in hbond_donor_dict)
        ...
        higlight_atoms = highlighted_hbond_donor + highlighted_hbond_acceptor + highlighted_hbond_both + highlighted_hydrophobic + highlighted_waterbridge + highlighted_pistacking + highlighted_halogen + highlighted_saltbridge + highlighted_pication + highlighted_metal 
        ...
        #drawer.DrawMolecule(prepared_ligand, highlightAtoms=highlighted_hydrophobic,  highlightAtomColors={k: (0.9, 0.9, 0.1)for k in highlighted_hydrophobic})
